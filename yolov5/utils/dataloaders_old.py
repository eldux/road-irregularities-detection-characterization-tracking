    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            for _ in range(self.vid_stride):
                ...
                if ret_val == sl.ERROR_CODE.SUCCESS:
                    ...
                    if ret_val == sl.ERROR_CODE.SUCCESS:
                        ...
                        #xyz = point_cloud_np[:, :, :3].reshape(-1, 3)  # Extract XYZ
                        #rgba = point_cloud_np[:, :, 3].reshape(-1)  # Extract RGBA float values

                        # Remove NaNs and invalid points
                        #valid_mask = ~np.isnan(xyz).any(axis=1)
                        #xyz = xyz[valid_mask]
                        #rgba = rgba[valid_mask]

                        # Convert RGBA to RGB
                        #rgb = float_to_rgb(rgba)

                        #if xyz.size > 0 and self.frame % 30 == 0:
                        #    pcd = o3d.geometry.PointCloud()
                        #    pcd.points = o3d.utility.Vector3dVector(xyz)
                        #    pcd.colors = o3d.utility.Vector3dVector(rgb)  # Assign real colors

                            # Run Open3D blocking mode visualization
                    #    o3d.visualization.draw_geometries([pcd], window_name="ZED RGB Point Cloud")
                    else:
                        point_cloud_np = None
                else:
                    raise StopIteration
            ...
            while ret_val != sl.ERROR_CODE.SUCCESS:
                ...
                path = self.files[self.count]
                self._new_svo(path)
                ret_val = self.cam.retrieve_image(image, sl.VIEW.LEFT, sl.MEM.CPU)  # retrieve image left and right
                if ret_val == sl.ERROR_CODE.SUCCESS:
                    im0 = image.get_data()
                    # Ensure OpenCV-compatible BGR format
                    if im0.shape[2] == 4:  # If RGBA, drop the alpha channel
        ...
        if self.svo_flag:
            return path, im, im0, self.cam, s, depth_np, point_cloud_np, timestamp
        else:
            return path, im, im0, self.cap, s, None, None, None
