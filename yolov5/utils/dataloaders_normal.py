...
from pyzed.sl import FLIP_MODE
...
SVO_FORMATS = "svo", "svo2"  # include video suffixes
...
def float_to_rgb(float_array):
    """Extracts RGB colors from ZED's 32-bit float RGBA format."""
    int_array = float_array.view(np.uint32)  # Convert float to uint32
    r = ((int_array >> 16) & 255) / 255.0  # Extract red and normalize
    g = ((int_array >> 8) & 255) / 255.0   # Extract green and normalize
    b = (int_array & 255) / 255.0          # Extract blue and normalize
    return np.stack((b, g, r), axis=-1)    # Combine into RGB array
...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        svos = [x for x in files if x.split(".")[-1].lower() in SVO_FORMATS]
        ni, nv, ns = len(images), len(videos), len(svos)

        self.img_size = img_size
        self.stride = stride
        self.files = images + videos + svos
        self.nf = ni + nv + ns # number of files
        self.video_flag = [False] * ni + [True] * nv + [False] * ns
        self.svo_flag = [False] * ni + [False] * nv + [True] * ns
        self.mode = "image"
        self.auto = auto
        ...
        if any(videos):
            ...
            self.cap = None
        if any(svos):
            self._new_svo(svos[0])  # new video
        else:
            self.cam = None
        assert self.nf > 0, (
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
        ...
        elif self.svo_flag[self.count]:
            # Read svo video
            self.mode = "svo"
            image = sl.Mat()
            #resolution = self.cam.get_camera_information().camera_configuration.resolution
            #low_resolution = sl.Resolution(min(640, resolution.width), min(360, resolution.height))
            self.runtime = sl.RuntimeParameters()
            ret_val = None

            for _ in range(self.vid_stride):
                ret_val = self.cam.grab(self.runtime)
                if ret_val == sl.ERROR_CODE.SUCCESS:
                    self.cam.retrieve_image(image, sl.VIEW.LEFT, sl.MEM.CPU)  # retrieve image left and right
                    timestamp = self.cam.get_timestamp(sl.TIME_REFERENCE.IMAGE).get_microseconds() / 1e6
                    im0 = image.get_data()

                    # Ensure OpenCV-compatible BGR format
                    if im0.shape[2] == 4:  # If RGBA, drop the alpha channel
                        im0 = cv2.cvtColor(im0, cv2.COLOR_RGBA2RGB)

                    # Retrieve the depth map
                    depth = sl.Mat()
                    ret_val = self.cam.retrieve_measure(depth, sl.MEASURE.DEPTH)
                    if ret_val == sl.ERROR_CODE.SUCCESS:
                    ...
                    point_cloud = sl.Mat()
                    ret_val = self.cam.retrieve_measure(point_cloud, sl.MEASURE.XYZRGBA)
                    if ret_val == sl.ERROR_CODE.SUCCESS:
                        # Convert point cloud to numpy array
                        point_cloud_np = point_cloud.get_data().copy()
                    else:
                        point_cloud_np = None

                    #Get position tracking results
                    camera_pose = sl.Pose()
                    py_translation = sl.Translation()

                    # Get the position of the camera in a fixed reference frame (the World Frame)
                    tracking_state = self.cam.get_position(camera_pose, sl.REFERENCE_FRAME.CAMERA)
                    tracking_status = self.cam.get_positional_tracking_status()

                    # Get rotation and translation and displays it
                    if tracking_state == sl.POSITIONAL_TRACKING_STATE.OK:
                        rotation = camera_pose.get_rotation_matrix().r
                        translation = camera_pose.get_translation(py_translation).get()
                else:
                    raise StopIteration

            while ret_val != sl.ERROR_CODE.SUCCESS:
                speed = 0
                self.count += 1
                self.cam.close()
                if self.count == self.nf:  # last video
                ...
                path = self.files[self.count]
                self._new_svo(path)
                ret_val = self.cam.retrieve_image(image, sl.VIEW.LEFT, sl.MEM.CPU)  # retrieve image left
                timestamp = self.cam.get_timestamp(sl.TIME_REFERENCE.IMAGE).get_microseconds() / 1e6
                if ret_val == sl.ERROR_CODE.SUCCESS:
                    im0 = image.get_data()
                    # Ensure OpenCV-compatible BGR format
                ...
                depth= sl.Mat()
                ret_val = self.cam.retrieve_measure(point_cloud, sl.MEASURE.DEPTH)
                if ret_val == sl.ERROR_CODE.SUCCESS:
                    # Convert point cloud to numpy array
                    depth_np = depth.get_data().copy()
                else:
            ...
            self.frame += 1
            # im0 = self._cv2_rotate(im0)  # for use if cv2 autorotation is False
            s = f"svo {self.count + 1}/{self.nf} ({self.frame}/{self.frames}) {path}: "
        ...
        if self.svo_flag:
            return path, im, im0, self.cam, s, depth_np, point_cloud_np, rotation, translation, timestamp, self.frame,
        else:
            return path, im, im0, self.cap, s, None, None, None, None, None, None, self.frame
    ...
    def _new_svo(self, path):
        ...
        init_params = sl.InitParameters(input_t=input_type,
                                        svo_real_time_mode=False, # Disable real-time mode for SVO playback
                                        camera_image_flip=FLIP_MODE.ON) # Adjust for flipping if required
        ...
        self.cam = sl.Camera() # Create ZED Camera instance
        status = self.cam.open(init_params) # Open SVO file

        if status != sl.ERROR_CODE.SUCCESS:  # Ensure the camera opened succesfully
            print(f"Error opening SVO file {status}. Exit program.")
            exit(1)
        ...
        # 3) Wrap into an sl.Mat (single‐channel 8‑bit)
        mask_roi = sl.Mat(width, height, sl.MAT_TYPE.U8_C1, sl.MEM.CPU)
        mask_data = mask_roi.get_data()
        mask_data[:, :] = 0
        y0, y1 = 0, height - int(height*0.3)
        x0, x1 = 0, width
        mask_data[y0:y1, x0:x1] = 255

        # 4) Tell the camera to use this custom ROI for depth AND tracking
        self.cam.set_region_of_interest(mask_roi,
                                        [sl.MODULE.POSITIONAL_TRACKING])  # applies mask in‑place :contentReference[oaicite:0]{index=0}

        # Set the total number of frames and resolution
        self.frames = int(self.cam.get_svo_number_of_frames() / self.vid_stride)
        self.orientation = 0

        # Get resolution details from the camera
        resolution = self.cam.get_camera_information().camera_configuration.resolution
        ...
        tracking_parameters.depth_min_range = 3
        tracking_parameters.enable_pose_smoothing = False
        tracking_parameters.mode = sl.POSITIONAL_TRACKING_MODE.GEN_1
        status = self.cam.enable_positional_tracking(tracking_parameters)
        if status != sl.ERROR_CODE.SUCCESS:  # Ensure the camera opened succesfully
            print(f"Error enable positional tracking: {status}. Exit program.")
            exit(1)

        print(f"Initialized SVO file: {path}")
        print(f"Total Frames: {self.frames}, Resolution: {self.width}x{self.height}")
